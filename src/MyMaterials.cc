...
{
  G4double a, z, a2, z2, density;

  G4Element* Cu = new G4Element("Copper",    "Cu", z=29., a=63.546*g/mole);
  G4Element* Zn = new G4Element("Zinc",    "Zn", z2=30., a2=65.409*g/mole);
...
G4Material* MyMaterials::LUAGCE()  // Lutetium Aluminum Garnet
{
  ...
  G4double ABS_LENGTH[NUMENTRIES_2]       = { 150.*mm, 150.*mm, 150.*mm };	//138 original
...
//   mt->AddProperty("RAYLEIGH",      ABS_Energy,  Rayleigh,     NUMENTRIES_2);
  mt->AddConstProperty("SCINTILLATIONYIELD",1000/MeV);
  ...
  // for now i am using the LSO scintillation properties
  const G4int LuAGCE_SCY_NUMENTRIES = 12;
  G4double LuAGCE_SCY_Energy[LuAGCE_SCY_NUMENTRIES] = { 0.000*MeV, 0.015*MeV, 0.020*MeV, 0.030*MeV, 0.040*MeV, 0.060*MeV, 
                                                  0.080*MeV, 0.090*MeV, 0.105*MeV, 0.300*MeV, 0.500*MeV, 1.000*MeV };
  G4double LuAGCE_SCY[LuAGCE_SCY_NUMENTRIES]        = { 0.10, 0.46, 0.60, 0.68, 0.74, 0.80, 0.82, 
                                                  0.84, 0.87,  0.96,  0.98,  1.00 };
  for(int i=0;i<LuAGCE_SCY_NUMENTRIES;i++) {
//     LuAGCE_SCY[i]=10000.0*MeV*LuAGCE_SCY[i]*LuAGCE_SCY_Energy[i];
    LuAGCE_SCY[i]=1000.0*MeV;
  }
  
  mt->AddProperty("ELECTRONSCINTILLATIONYIELD",LuAGCE_SCY_Energy,LuAGCE_SCY,LuAGCE_SCY_NUMENTRIES);
